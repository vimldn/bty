  ...
  return (
    <div className="relative flex items-center justify-center w-16 h-16 shrink-0">
      <svg width="64" height="64" viewBox="0 0 64 64" className="-rotate-90">
        ...
        <circle
          cx="32" cy="32" r={r}
          strokeWidth="4" stroke={color} fill="none"
...
function OxidationSwatch({ lab, label }: { lab: { L: number; a: number; b: number }; label: string }) {
  const bg = `hsl(${28 + lab.b * 1.5}, ${20 + lab.a * 2}%, ${lab.L * 0.9}%)`;
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="w-10 h-10 rounded-lg border border-white/20" style={{ backgroundColor: bg }} />
...
export function MatchCard({ result, rank }: { result: ShadeResult; rank: number }) {
  ...
  const freshBg = `hsl(${30 + result.oxidation.fresh.b * 1.5}, ${
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
      ...
      >
        <span className="text-3xl font-black text-white/20 w-6 shrink-0 text-center">
          {rank === 1 ? "★" : rank}
        </span>

        <div
          className="w-12 h-12 rounded-xl shrink-0 border border-white/20"
          style={{ backgroundColor: freshBg }}
        />
        ...
        <div className="px-4 pb-4 flex flex-col gap-4 border-t border-white/5 pt-4">
          ...
          <div>
            <div className="flex justify-between text-[11px] text-white/50 mb-1">
              ...
              <span className="capitalize">{result.undertoneClass.replace("_", " ")}</span>
            </div>
          ...
          <div className="flex gap-3 items-center">
            <OxidationSwatch lab={result.oxidation.fresh}    label="Fresh"   />
            <span className="text-white/20 text-lg">→</span>
            <OxidationSwatch lab={result.oxidation.oxidized} label="2hr Wear"/>
          ...
          <a
            ...
            className="block w-full text-center py-3 rounded-xl bg-violet-600 hover:bg-violet-500 active:bg-violet-700 text-white font-semibold text-sm transition-colors"
          >
            Shop This Shade →
          </a>
        </div>
