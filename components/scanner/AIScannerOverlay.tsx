...
function Bar({ label, score, warn }: { label: string; score: number; warn?: boolean }) {
  ...
  const color = warn || pct < 50 ? "bg-red-500" : pct < 80 ? "bg-amber-400" : "bg-emerald-400";
  return (
...
export function AIScannerOverlay({ lightingScore, positionScore, shadowWarning, liqaPass, scanProgress, status }: Props) {
  const msg =
    ...
    liqaPass            ? "Hold still — scanning…"       : "Adjusting…";
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-between p-6 pointer-events-none">
      ...
      <div
        ...
        style={{
          ...
          boxShadow:   liqaPass ? "0 0 0 4px rgba(74,222,128,0.25)" : "0 0 0 4px rgba(251,191,36,0.25)",
        }}
      />

      <div className="relative z-10 bg-black/60 backdrop-blur-sm rounded-xl px-4 py-2 mt-2">
        <p className="text-white text-sm font-medium text-center tracking-wide">{msg}</p>
      </div>
